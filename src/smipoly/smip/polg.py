...
def biplym(df, targ=None, Pmode=None, dsp_rsl=None):
            ...
            if len(temp1) != 0:
                if targ_mon2 != "none":
                    ...
                    if len(temp2) != 0:
                        ...
                        DF_temp["ROMol1"] = DF_temp["mon1"].apply(
                            Chem.MolFromSmiles
                        )
                        DF_temp["ROMol2"] = DF_temp["mon2"].apply(
                            Chem.MolFromSmiles
                        )
                        DF_temp["reactant"] = DF_temp.loc[
                            :, ["ROMol1", "ROMol2"]
                        ].values.tolist()
                        targ_rxn = P_set[2]
                        if Pmode == "r":
                            DF_temp["ROPolym"] = DF_temp["reactant"].apply(
                                bipolymR,
                                targ_rxn=targ_rxn,
                                monL=monL,
                                Ps_rxnL=Ps_rxnL,
                    ...
                    if Pmode == "r":
                        DF_temp["ROPolym"] = DF_temp["ROMol1"].apply(
                            homopolymR,
                            mons=mons,
                            excls=excls,
                            targ_mon1=targ_mon1,
    ...
    DF_gendP2["reactset"] = np.sort(
        DF_gendP2.loc[:, ["mon1", "mon2"]].values
    ).tolist()
